...
class Kemono:
    ...
    async def _download_attachment(self, attachment: Attachment, dst_dir: Path, post_id: str, idx: int) -> bool:
        ...
        expected_sha256 = sha256_from_path(attachment.path)
        desc = f'{post_id}-{idx}'
        attempts = len(_API_RETRY_DELAYS_SECONDS) + 1
        for attempt in range(1, attempts + 1):
            await self._pace()
            ...
            if outcome.status == 'missing':
                log.warning('Attachment missing upstream (404): %s', attachment.path)
                return False
            if attempt >= attempts:
                msg = f'Download of {attachment.path} failed after {attempts} attempts: {outcome.detail}'
                raise RuntimeError(msg)
        ...
        msg = f'Download of {attachment.path} failed'
        raise RuntimeError(msg)

    async def _download_post(self, post: Post, creator_dir: Path) -> tuple[int, int]:
        """Returns (downloaded, missing) attachment counts. Transient failures propagate
        so the caller can abort the creator and resume here on the next run."""
        ...
        for idx, attachment in enumerate(post.attachments, start=1):
            if await self._download_attachment(attachment, post_dir, post.id, idx):
                downloaded += 1
            else:
                missing += 1
    ...
    async def _update_creator(self, creator: KemonoCreator) -> _CreatorStats:
        ...
        for idx, post in enumerate(reversed(new_posts), start=1):
            ...
            downloaded, missing = await self._download_post(post, creator_dir)
            await database.query_db(
    ...
    async def update(self) -> None:
        ...
        await self._notify_summary(totals, missing_creators, failed)
        if failed:
